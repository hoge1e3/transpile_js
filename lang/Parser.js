...
*/
	...
	parseTokens: function (tokens,global) {
...
function State(strOrTokens, global) { // class State
	if (strOrTokens!=null) {
		...
		if (typeof strOrTokens=="string") {
			...
			var origin=1;
			this.src.pos2rc=new Pos2RC(this.src.str,origin);
			this.src.maxRow=this.src.maxCol=origin;
		}
...
extend(State.prototype, {
	...
	updateMaxPos:function (npos) {
		if (npos > this.src.maxPos) {
			...
			if (this.src.pos2rc) {
				var rc=this.src.pos2rc.getRC(npos);
				this.src.maxRow=rc.row;
				this.src.maxCol=rc.col;
			}
		}
	},
	isSuccess: function () {
...
if (typeof window==="object") window.Parser=$;
//export default $;
return $;
});
