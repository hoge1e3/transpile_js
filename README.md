...
このParser(`p`)が1個以上繰り返されるという構文を解析する新しいParserオブジェクトを返します．`result`の形式はrep0と同じです．
...
- `g.rep0(構文式)`  : 構文式が0回以上繰り返される構文を表します．解析結果は構文木の配列になります．rep1も同様に書けます．
...
- printf(format, ...params)
  ...
  - 書式文字列に指定できるのは次のものです
    - `%s` 文字列を出力します．文字列以外のものについてはString.valueOfで変換します（字句ノードは自動的にもとのソースコード内の文字列に変換されます）
